...
if ($_SESSION['employee'] == 'yes') {
    $output = '';
    $sql = mysqli_query($conn, "select * from employee_document where e_id = '" . $_SESSION['e_id'] . "' AND admin_id = '" . $_SESSION['admin_id'] . "' ORDER BY file_added_time DESC ");
    while ($row = mysqli_fetch_array($sql)) {
        if ($row['file_extension'] == 'pdf') {
            ...
            $status = '<div class="card-file-thumb-png">
                                        <img src="uploads/' . $row['file_name'] . '" height="120px" width="100%">      
                                    </div>';
        }
        
        time_Ago($row['file_added_time']);

        $output .= '<div class="col-6 col-sm-4 col-md-3 col-lg-4 col-xl-3"> <div class="card card-file">' . $status . '<div class="card-body">
                                    <h6>' . $row['file_name'] . '</h6>
                                    <span>' . $row['file_size'] / 1000 . ' KB</span>
                                </div>
