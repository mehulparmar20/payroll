...
class CheckGenerator {
	...
	function PrintChecks() {
		...
		foreach( $this->checks as $check ) {
			...
			// date
			//$pdf->Line( $x + 0, $y + 0, $x + 0 + 0, $y + 0 );
			$pdf->SetXY( $x + 3.5, $y + .48, 1.1, .25 );
			$date_str = $this->matchcase($check['from_name'],"");
			$pdf->Cell( 1, (7/72), $date_str );

			// pay to the order of
			//$pdf->Line( $x + $cell_left, $y + 0, $x + $cell_left + 0, $y + 0 );
			$pdf->SetXY( $x + $cell_left, $y + 0);
			$pay_str = $this->matchcase($check['from_name']," ");
			...
			// dollars
			//$pdf->Line( $x + $cell_left, $y + 0, $x + $cell_left + 0, $y + 0 );
			$pdf->SetXY( $x + $cell_left + 0, $y + 0);
			$dollar_str = $this->matchcase($check['from_name']," ");
			...
			// amount
			if( $check['amount'] > 0 ) {
				$Rupees = intval($check['amount']);
				$cents = round(($check['amount'] - $Rupees)*100);
				//$dollars_str = TextualNumber::GetText($dollars);
				$numtxt = new TextualNumber($Rupees);
				$Rupees_str = $numtxt->numToWords($Rupees);
				
				$amt_string = "**".ucfirst(strtolower($Rupees_str))." Rupees";
				if( $cents > 0 ) {
					$amt_string .= "";
				} else {
					$amt_string .= "";
				...
				$pdf->SetXY( 1.20 ,$y + 0.85);
				$pdf->Cell( 0, 0.90, $amt_string,0,0,'L');
                                $pdf->SetXY( $x + 4, $y + 1.40);
                                $pdf->Cell( 1, 0.50, $amt ,0,0,'R');
